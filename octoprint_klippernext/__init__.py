...
class KlipperNextPlugin(
	...
	def load_klipper_config(self):
		...
		try:
			...
			config = f.read()
			f.close()
			self._logger.debug("Klipper configuration has been loaded")
		except IOError:
			self._logger.error(
	...
	def get_template_configs(self):
		return [
			dict(
				type="navbar",
				template="klipper_navbar.jinja2",
				custom_bindings=True
			),
			dict(
				type="settings",
				template="klipper_settings.jinja2",
				custom_bindings=True
			),
			dict(
				type="generic",
			...
			dict(type="sidebar",
				template="klipper_sidebar.jinja2",
				custom_bindings=True,
				icon="rocket",
		...
		if "FIRMWARE_VERSION" in line:
			...
			if "!!" in line:
				...
				if self._wait_save_config:
					self._wait_save_config = False
					self._logger.debug("Got SAVE_CONFIG response. Reload Klipper config.")
					config = self.load_klipper_config()
					if config:
	...
	def on_parse_gcode_sent(self, comm, phase, cmd, cmd_type, gcode, subcode=None, tags=None, *args, **kwargs):
		# self._logger.debug("Sent command: {phase}, {cmd}, {cmd_type}, {gcode}".format(**locals()))
		if cmd == "SAVE_CONFIG":
			self._wait_save_config = True
			self._logger.debug("Wait SAVE_CONFIG response")
		return None
	...
	def get_update_information(self):
		return dict(
			klipper=dict(
				...
				user="antage",
				repo="octoklipper-next",
				pip="https://github.com/antage/octoklipper-next/archive/{target}.zip"
			)
		)
...
def __plugin_load__():
	...
	__plugin_implementation__ = KlipperNextPlugin()
	__plugin_hooks__ = {
		"octoprint.comm.protocol.gcode.received": __plugin_implementation__.on_parse_gcode,
		"octoprint.comm.protocol.gcode.sent": __plugin_implementation__.on_parse_gcode_sent,
